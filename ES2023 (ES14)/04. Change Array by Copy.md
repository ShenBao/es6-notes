...
TypedArray是一种通用的固定长度缓冲区类型，允许读取缓冲区中的二进制数据。其在 WEBGL 规范中被引入用于解决Javascript处理二进制数据的问题。类型化数组也是数组，只不过其元素被设置为特定类型的值。
...
同时该提案还引入了一个无损方法 .with()，不过该方法没有对应破环版本。该方法的作用是无损的替换某个值（有点类似于arr[index]=value）
